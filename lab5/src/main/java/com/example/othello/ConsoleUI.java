...
import com.example.othello.chess.ChessSide;
...
public class ConsoleUI implements AutoCloseable {
    ...
    public void render(
        ...
        render(session, currentGameNumber, sessions, legalMoves, hintMessage, promptText, "");
    }
    ...
    public void render(
        ...
        List<String> rightColumn = buildGameListColumn(sessions, currentGameNumber, promptText);
        ...
        if (demoInfo != null && !demoInfo.isBlank()) {
            List<String> demoLines = wrapText("Demo Info: " + demoInfo, LEFT_WIDTH + CENTER_WIDTH + 18);
            for (String demoLine : demoLines) {
                putLine(line++, demoLine);
            }
        }
        putLine(line++, "");
    ...
    private List<String> buildBoardColumn(GameSession session, Set<Position> legalMoves) {
        ...
        if (session.getGameType() == GameType.CHESS) {
            return buildChessBoardColumn(session);
        }
    ...
    private List<String> buildCenterColumn(GameSession session, int currentGameNumber, String hintMessage) {
        ...
        if (session.getGameType() == GameType.CHESS) {
            return buildChessCenterColumn(session, currentGameNumber, hintMessage);
        }
    ...
    private List<String> buildChessBoardColumn(GameSession session) {
        List<String> lines = new ArrayList<>();
        ChessBoard board = session.getChessBoard();
        lines.add("Chess Board");
        lines.add(buildHeader(board.getSize()));
        ...
        for (int row = 0; row < board.getSize(); row++) {
            ...
            line.append(row + 1).append(" ");
            for (int col = 0; col < board.getSize(); col++) {
                line.append(board.displaySymbol(new Position(row, col))).append(" ");
            }
            lines.add(line.toString());
        }

        lines.add("");
        lines.add("Black: K Q R B N P");
        lines.add("White: k q r b n p");
        lines.add(".: empty");
        return lines;
    }

    private List<String> buildChessCenterColumn(
        GameSession session,
        int currentGameNumber,
        String hintMessage
    ) {
        List<String> lines = new ArrayList<>();
        ChessSide side = ChessRules.sideFromCell(session.getCurrentPlayer());

        lines.add("Game " + currentGameNumber);
        lines.add("Mode: chess");
        lines.add("White lowercase" + chessTurnMarker(side, ChessSide.WHITE, session));
        lines.add("Black uppercase" + chessTurnMarker(side, ChessSide.BLACK, session));
        lines.add("Current side: " + side.displayName());
        lines.add("Win: capture opponent king");
        lines.add("Special: castling, en passant,");
        lines.add("promotion");
    ...
    private List<String> buildGameListColumn(List<GameSession> sessions, int currentGameNumber, String promptText) {
        List<String> lines = new ArrayList<>();
        lines.add("Game List");
        ...
        lines.add("");
        lines.add("Commands");
        for (String command : promptText.replace(">", "").split("\\|")) {
    ...
    private String chessTurnMarker(ChessSide currentSide, ChessSide targetSide, GameSession session) {
        if (session.isFinished() || currentSide != targetSide) {
            return "";
        }
        return " <- turn";
    }
